...
use App\Http\Controllers\ApiControllerPembelian;
use Artisan;
use Cookie;
use JWTAuth;
...
class DataPembelianAll implements FromView, WithColumnFormatting
{
    public $request;
    ...
    public function view(): View
    {
    	...
    	if(empty(session('key_token_aneka'))){
            ...
            return view('admin.AdminOne.menupembelian.exportdata.historypembelianitem',['url_api' => $url_api,'app' => 'pembelianmenu','url_active' => 'pembelianbarang','request' => $request,'res_user' => $res_user,'results' => $results['results']['list'],'listdata' => $results['results']]);
        }
    }
    ...
    public function columnFormats(): array
    {
        ...
        return [
            'k' => '#,##0.00', // qty pembelian
            'L' => '#,##0.00', // qty diterima
            'M' => '#,##0.00', // qty belum diterima
            'O' => '#,##0.00', // harga
            'P' => '#,##0.00', // diskon
            'Q' => '#,##0.00', // ppn
            'R' => '#,##0.00', // total
