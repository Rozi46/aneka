<?php
    ...
    $response = app(\App\Services\ApiServicePenjualan::class)->viewpenjualan($request);
    $data = is_array($response) ? $response : $response->getData(true);

    if(($data['status_message'] ?? 'Belum ditentukan') == 'failed'){
        echo "<meta http-equiv='refresh' content='0;/'>";
        exit;
    }
    ...
    class PDF extends MultiCellTable
    {
        ...
        function Header()
        {
            ...
            $this->Ln(3);
            $this->Cell(45);
            $this->Cell(0,5,strtoupper($getdata['detail_perusahaan']['kantor'] ?? 'Belum ditentukan'),0,0,'L');

            $this->Ln(6);
            $this->SetFont('Arial','',9);
            $this->Cell(45);
            $this->MultiCell(210,5,ucwords(strtolower($getdata['detail_perusahaan']['jenis'] ?? 'Belum ditentukan')),0,'L');

            $this->Cell(45);
            $this->Cell(0,5,ucwords(strtolower($getdata['detail_perusahaan']['alamat'] ?? 'Belum ditentukan')),0,0,'L');

            $this->Ln();
            $this->Cell(45.5);
            $this->Cell(0,5,'Email : '.($getdata['detail_perusahaan']['email'] ?? 'Belum ditentukan'),0,0,'L');
            ...
            $this->SetFont('Arial','B',14);
            $this->Cell(0,5,'SALES ORDER',0,0,'C');
            ...
            $mekanik = [];
            foreach ($getdata['list_mekanik'] ?? [] as $m) {
                $code = $m['code_mekanik'];
                if(isset($getdata['detail_mekanik'][$code])){
                    $mekanik[] = $getdata['detail_mekanik'][$code]['nama'];
                }
            }

            $namaMekanik = !empty($mekanik) ? implode(', ', $mekanik) : 'Belum ditentukan';

            $this->SetFont('Arial','',9);

            $this->Cell(6); 
            $this->Cell(94,$seth,'Customer :',0,0);
            $this->Cell(25,$seth,'Date',0,0);
            $this->Cell(3,$seth,':',0,0);
            $this->Cell(0,$seth,isset($getdata['detail']['tanggal']) ? date('j F Y', strtotime($getdata['detail']['tanggal'])) : 'Belum ditentukan',0,1);

            $this->SetFont('Arial','B',9);
            $this->Cell(6); 
            $this->Cell(94,$seth,($getdata['detail_customer']['nama'] ?? 'Belum ditentukan'),0,0);

            $this->SetFont('Arial','',9);
            $this->Cell(25,$seth,'Sales No',0,0);
            $this->Cell(3,$seth,':',0,0);
            $this->Cell(0,$seth,($getdata['detail']['nomor'] ?? 'Belum ditentukan'),0,1);
            ...
            $this->Cell(6);
            $this->Cell(94,$seth,'Phone No : '.($getdata['detail_customer']['no_telp'] ?? '-'),0,1);
        ...
        function Footer()
        {
            ...
            $this->Ln(1);
            $this->Cell(12);
            $this->MultiCell($setw70 + 30,$seth,($getdata['detail']['ket'] ?? 'Belum ditentukan'),0);
            $setmulti_h = $this->GetY();
            $setmulti_h= $setmulti_h-$gety;
            $getx+=$setw40;
            $this->SetXY($getx, $gety);
            ...
            // $this->Cell(34,5,number_format($getdata['detail']['biaya_kirim'] ?? 0,2,",","."),0,0,'R'); 
            ...
            $this->Line(139,$gety-5,139,234.5); // Garis tegak grand total
            $this->Line(197,$gety-5,197,259.2);
            $this->Line(11.5,$gety+46.5,197,$gety+46.5); // Garis dibawah shipping adress
        ...
        function AddContent()
        {
            ...
            foreach ($getdata['list_produk'] as $view_data) {
                ...
                $qty = number_format($view_data['jumlah_jual'] ?? 0,0,"",".").' '.($getdata['satuan_produk'][$kode_barang]['nama'] ?? '');
